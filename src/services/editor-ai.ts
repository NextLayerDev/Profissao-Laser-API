...
class EditorAiService {
	...
	// Reusa o mesmo cliente OpenRouter já usado em /previas/generate e /editor/ai.
	// Zero binary nativo, zero WASM, zero env vars novas. Funciona em qualquer
	// base de Node (alpine, slim, etc.) porque é só uma chamada HTTP.
	async removeBackground(
		...
		try {
					...
					messages: [
						{
							role: 'user',
							content: [
								{ type: 'text', text: createRemoveBackgroundPrompt() },
								{
									type: 'image_url',
									image_url: { url: `data:image/png;base64,${base64}` },
								},
			...
			// Tenta pegar do campo images primeiro (Gemini 3 Pro Image)
			if (message?.images && message.images.length > 0) {
				const imageData = message.images[0];
				if (imageData.image_url && typeof imageData.image_url === 'object') {
					result = (imageData.image_url as { url: string }).url;
			...
			// Fallback: parse markdown/URL no content
			if (!result && message?.content) {
				const markdownMatch = message.content.match(
					/!\[.*?\]\((https?:\/\/[^)]+)\)/,
				);
			...
			if (!result) {
				throw new Error('IA não retornou imagem para remoção de fundo.');
			}

			// Normalizar formato de saída
			let finalBase64: string;
			if (result.startsWith('data:image')) {
				finalBase64 = result;
			} else if (result.startsWith('http')) {
