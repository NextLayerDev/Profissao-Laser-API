...
export function summarizeDoc(doc: ToolDefinitionDoc): string {
	...
	if (doc.room) {
		...
		return [
			...
			`- Aparência: aluno=${r.ui?.customer ? 'personalizada' : 'padrão'}, admin=${r.ui?.admin ? 'personalizada' : 'padrão'} (use set_room_ui)`,
		].join('\n');
	}
